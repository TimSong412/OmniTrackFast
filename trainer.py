...
class BaseTrainer():
    ...
    def read_data(self):
        ...
        if "RGB" in self.args.data_dir:
            ...
            annotation_file = "{}/{}.npy".format(annotation_dir, self.seq_name)
    ...
    def eval_one_step(self, step, occlusion_th=0.99):
        print(f"eval_step: {step}")

        out_dir = os.path.join(self.out_dir, 'eval')
        ...
        query_points = self.query_points.copy()
        target_points = self.target_points.copy()
        gt_occluded = self.gt_occluded.copy()
        evaluation_points = self.evaluation_points.copy()
        ids1 = self.eval_ids1.copy()
        px1s = self.eval_px1s.clone()

        results = np.zeros(target_points.shape)
        occlusions = np.zeros(gt_occluded.shape)
        with torch.no_grad():
            for i in range(gt_occluded.shape[-1]):
                results_, occlusions_ = self.get_correspondences_and_occlusion_masks_for_pixels(
                    ids1=ids1, 
        ...
        target_points *= (self.w / 256, self.h / 256)
        
        results[..., 0] *= self.eval_w / self.w
        results[..., 1] *= self.eval_h / self.h

        target_points[..., 0] *= self.eval_w / self.w
        target_points[..., 1] *= self.eval_h / self.h


        metrics = {}

        occlusion_mask = occlusions > occlusion_th
        out_of_boundary_mask = (results[..., 0] < 0) | (results[..., 0] > self.w - 1) | \
                                (results[..., 1] < 0) | (results[..., 1] > self.h - 1)
        ...
        for k, v in metrics.items():
            self.scalars_to_log['metric/{}'.format(k)] = v.item()
