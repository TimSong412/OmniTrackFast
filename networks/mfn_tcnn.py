...
class MFNBase(nn.Module):
    ...
    def __init__(
        ...
        self.linear = nn.ModuleList(
            [tcnn.Network(
                n_input_dims=hidden_size,
                n_output_dims=hidden_size,
                network_config={
                    "otype": "CutlassMLP",
...
class GaborLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        res = torch.sin(self.linear(x)) * torch.exp(-0.5 * D * self.gamma[None, :])
        return res.reshape(*xshape[:-1], -1)
