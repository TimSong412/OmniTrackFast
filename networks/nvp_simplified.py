...
class FixedPositionalEncoding(ProjectionLayer):
    def __init__(self, input_dims, frequency, proj_dims):
        ...
        self.proj = nn.Sequential(
            # nn.Linear(input_dims + input_dims * ll * 2, proj_dims), nn.LeakyReLU()  # orgin
    ...
    def forward(self, x):
        if x.ndim == 4:
            ...
            encoded = torch.cat(
                [
                    torch.sin(x[..., None] * self.sigma[None, None]),
                    torch.cos(x[..., None] * self.sigma[None, None]),
                ],
                dim=-1,
            ).view(*x.shape[:-1], -1)
