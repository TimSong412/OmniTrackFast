...
class SimpleDepthDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if id2 - id1 < 0:
            flow = self.flow[id1, id2 - id1 + 16].reshape(-1, 2)
            mask = self.masks[id1, id2 - id1 + 16].reshape(-1)
        else:
            flow = self.flow[id1, id2 - id1 + 15].reshape(-1, 2)
...
class RAFTDepthDataset(Dataset):
    def __init__(self, args, max_interval=None):
        ...
        sample_weights_dict = get_sample_weights(flow_stats)

        self.sample_weights = np.zeros((self.num_imgs, 32), dtype=np.float32)
        # for i, k in enumerate(sorted(sample_weights_dict.keys())):
        #     assert sample_weights_dict[k].keys().__len__() == sample_weights_dict.keys().__len__() - 1

        for i, k in enumerate(sorted(sample_weights_dict.keys())):
            for j in range(-16, 17):
                if j == 0 or i + j < 0 or i + j >= self.num_imgs:
                    continue
                if j < 0:
                    # kk = sorted(sample_weights_dict[k].keys())[i+j]
                    kk = sorted(sample_weights_dict[k].keys())[16+j-(32-len(sample_weights_dict[k].keys()))]
                    self.sample_weights[i, j+16] = sample_weights_dict[k][kk]
                else:
                    # kk = sorted(sample_weights_dict[k].keys())[i+j-1]
                    kk = sorted(sample_weights_dict[k].keys())[15+j-(32-len(sample_weights_dict[k].keys()))]
                    self.sample_weights[i, j+15] = sample_weights_dict[k][kk]
    ...
    def __getitem__(self, idx):
        ...
        sample_weights = self.sample_weights[id1, id2s - id1 + 16]
...
class RAFTExhaustiveDataset(Dataset):
    def __init__(self, args, max_interval=None):
        ...
        self.flow = np.zeros((self.num_imgs, 32, self.h, self.w, 2), dtype=np.float32)
        self.masks = np.zeros((self.num_imgs, 32, self.h, self.w, 3), dtype=np.float32)
        print("loading flow and mask")
        for i in tqdm.trange(self.num_imgs):
            for j in range(-16, 17):
                if j == 0:
...
class RAFTsimpleDataset(Dataset):
    def __init__(self, args, max_interval=None):
        self.args = args
        self.seq_dir = args.data_dir
        self.seq_name = os.path.basename(self.seq_dir.rstrip('/'))
